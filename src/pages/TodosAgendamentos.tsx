...
const TodosAgendamentos = () => {
  const { agendamentos, espacos, usuarios, loading, error, actions } =
    useSupabaseData();
  ...
  const filteredAgendamentos = useMemo(() => {
    ...
    if (
      ...
      filtered = filtered.filter(
        (a) => a.status === filters.filters.statusFilter
      );
    }
  ...
  // Paginação
  const pagination = usePagination(filteredAgendamentos, {
    initialPageSize: 10,
  });
  ...
  const getEspacoNome = (espacoId: number) => {
    return (
      espacos.find((e) => e.id === espacoId)?.nome || "Espaço não encontrado"
    );
  };

  const getUsuarioNome = (usuarioId: number) => {
    return (
      usuarios.find((u) => u.id === usuarioId)?.nome || "Usuário não encontrado"
    );
  };
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            {pagination.paginationInfo.totalItems} agendamentos
          </CardDescription>
        </CardHeader>
        <CardContent>
          <ResponsiveTable
                    ...
                    {agendamento.status === "pendente" && (
                      <div className="flex gap-2">
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleStatusChange(agendamento.id, "aprovado")
                          }
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleStatusChange(agendamento.id, "rejeitado")
                          }
