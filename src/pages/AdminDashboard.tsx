...
export default function AdminDashboard() {
  const { usuarios, espacos, agendamentos, agendamentosFixos, loading } =
    useSupabaseData();
  ...
  const stats = {
    ...
    agendamentosPendentes: agendamentos.filter((a) => a.status === "pendente")
      .length,
    agendamentosAprovados: agendamentos.filter((a) => a.status === "aprovado")
      .length,
    agendamentosRejeitados: agendamentos.filter((a) => a.status === "rejeitado")
      .length,
  ...
  const espacosUtilizacao = espacos
    .map((espaco) => {
      const agendamentosEspaco = agendamentos.filter(
        (a) => a.espacoId === espaco.id
      );
      return {
        ...espaco,
  ...
  return (
    <div className="space-y-6 p-6 bg-white dark:bg-background transition-colors">
      <PageHeader
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="border-green-200 bg-green-50 dark:bg-green-900/30 hover:shadow-lg transition-shadow">
          <CardContent className="p-6 text-center">
            ...
            <div className="text-xs text-green-500 mt-1">
              {(
                (stats.agendamentosAprovados / stats.totalAgendamentos) * 100 ||
                0
              ).toFixed(1)}
              % do total
            </div>
        ...
        <Card className="border-orange-200 bg-orange-50 dark:bg-orange-900/30 hover:shadow-lg transition-shadow">
          <CardContent className="p-6 text-center">
        ...
        <Card className="border-red-200 bg-red-50 dark:bg-red-900/30 hover:shadow-lg transition-shadow">
          <CardContent className="p-6 text-center">
            ...
            <div className="text-xs text-red-500 mt-1">
              {(
                (stats.agendamentosRejeitados / stats.totalAgendamentos) *
                  100 || 0
              ).toFixed(1)}
              % do total
            </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card
            className="hover:shadow-lg transition-all cursor-pointer group dark:bg-background dark:border-gray-700"
          ...
          >
            <CardContent className="p-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <ChevronRight className="w-4 h-4 text-gray-400 dark:text-gray-300 group-hover:translate-x-1 transition-transform" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card
            className="hover:shadow-lg transition-all cursor-pointer group dark:bg-background dark:border-gray-700"
            onClick={() => navigate("/aprovar-agendamentos")}
          >
            <CardContent className="p-6">
              <div className="flex items-center gap-3 mb-3">
                <div className="p-2 rounded-lg bg-purple-100 dark:bg-purple-900/30 group-hover:bg-purple-200 dark:group-hover:bg-purple-800/50 transition-colors">
                  <ClipboardCheck className="w-5 h-5 text-purple-600 dark:text-purple-300" />
                </div>
                <h3 className="font-semibold text-gray-900 dark:text-gray-100">
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  {stats.agendamentosPendentes} pendente
                  {stats.agendamentosPendentes !== 1 ? "s" : ""}
                </Badge>
                <ChevronRight className="w-4 h-4 text-gray-400 dark:text-gray-300 group-hover:translate-x-1 transition-transform" />
              </div>
            </CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Agendamentos Recentes */}
        <Card className="dark:bg-background dark:border-gray-700">
                ...
                {agendamentosRecentes.length > 0 ? (
                  agendamentosRecentes.map((agendamento) => {
                    const usuario = usuarios.find(
                      (u) => u.id === agendamento.usuarioId
                    );
                    const espaco = espacos.find(
                      (e) => e.id === agendamento.espacoId
                    );
                      ...
                      >
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-600 dark:text-gray-300 mb-1">
                            {usuario?.nome} •{" "}
                            {formatAulas(
                              agendamento.aulaInicio as NumeroAula,
                              agendamento.aulaFim as NumeroAula
                            )}
                          </p>
                ...
                espacosUtilizacao.map((espaco, index) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex items-center justify-center w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 font-semibold text-sm">
                        {index + 1}
                      </div>
                      <div>
                        <p className="font-medium text-sm text-gray-900 dark:text-gray-100">
                        ...
                        <div
                          ...
                          style={{
                            width: `${Math.min(
                              ...
                              (espaco.utilizacao /
                                Math.max(
                                  ...espacosUtilizacao.map((e) => e.utilizacao)
                                )) *
                                100
                            )}%`,
                          }}
