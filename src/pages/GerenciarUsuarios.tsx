...
const GerenciarUsuarios = () => {
  const { usuarios, agendamentos, espacos, loading, error, actions } =
    useSupabaseData();
  ...
  const filteredUsuarios = useMemo(() => {
    ...
    if (
      ...
      filters.filters.statusFilter !== "todos"
    ) {
      filtered = filtered.filter(
        (u) => u.tipo === filters.filters.statusFilter
      );
    }
    ...
    onSubmit: async (values) => {
      ...
      if (Object.keys(formErrors).length > 0) {
        Object.values(formErrors).forEach((error) =>
          notifications.error("Erro de validação", error)
        );
        return;
      }
  ...
  const validateBusinessRules = (values: UsuarioFormData): string[] => {
    ...
    if (editingUsuario?.tipo === "admin" && values.tipo !== "admin") {
      const outrosAdmins = usuarios.filter(
        (u) => u.tipo === "admin" && u.id !== editingUsuario.id
      );
    ...
    if (!ativo && usuario) {
      ...
      if (usuario.tipo === "admin") {
        const outrosAdminsAtivos = usuarios.filter(
          (u) => u.tipo === "admin" && u.ativo && u.id !== usuarioId
        );
  ...
  const handleConfirmDelete = async () => {
    ...
    if (usuarioToDelete.tipo === "admin") {
      const outrosAdmins = usuarios.filter(
        (u) => u.tipo === "admin" && u.id !== usuarioToDelete.id
      );
      if (outrosAdmins.length === 0) {
    ...
    const agendamentosUsuario = agendamentos.filter(
      (a) => a.usuarioId === usuarioToDelete.id
    );
    if (agendamentosUsuario.length > 0) {
      errors.push(
        `Este usuário possui ${agendamentosUsuario.length} agendamento(s) no sistema`
      );
    }
  ...
  const getAgendamentosCount = (usuarioId: number) => {
    return agendamentos.filter(
      (a) => a.usuarioId === usuarioId && a.status !== "rejeitado"
    ).length;
  };
    ...
    <div className="space-y-6 p-6">
              ...
              <form onSubmit={form.handleSubmit} className="space-y-4">
                <div className="grid gap-4 py-4">
                  ...
                  <div className="grid gap-2">
                    ...
                    >
                      <SelectTrigger
                        className={form.errors.tipo ? "border-red-500" : ""}
                      >
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="senha">
                      Senha{" "}
                      {editingUsuario ? "(deixe vazio para manter atual)" : "*"}
                    </Label>
                    <Input
                      id="senha"
                      type="password"
                  ...
                  {form.values.tipo === "gestor" && (
                    <div className="grid gap-2">
                      ...
                      <div className="text-sm text-gray-600 mb-2">
                        Selecione os espaços que este gestor poderá aprovar
                        agendamentos
                      </div>
                        ...
                        {espacos
                          ...
                          .map((espaco) => (
                            ...
                            >
                              <Checkbox
                                id={`espaco-${espaco.id}`}
                                checked={form.values.espacos.includes(
                                  espaco.id
                                )}
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="sm:w-48">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  filters.updateFilter("statusFilter", value)
                }
              >
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Mostrando {pagination.paginationInfo.startItem} a{" "}
            {pagination.paginationInfo.endItem} de{" "}
            {pagination.paginationInfo.totalItems} usuários
          </CardDescription>
        </CardHeader>
        <CardContent>
          <ResponsiveTable
            ...
            columns={[
              ...
              },
              {
                ...
                accessor: (usuario) => {
                  ...
                  const espacosGestor = espacos.filter((e) =>
                    usuario.espacos?.includes(e.id)
                  );
                  return (
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Esta ação é <strong>irreversível</strong>. O usuário será
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {getAgendamentosCount(usuarioToDelete.id)}
                    </span>
                  </div>
                  {usuarioToDelete.tipo === "gestor" &&
                    usuarioToDelete.espacos?.length && (
                      <div className="flex justify-between">
