...
-- Create table: agendamentos (sem referência a agendamentos_fixos por enquanto)
CREATE TABLE agendamentos (
    ...
    observacoes TEXT,
    agendamento_fixo_id INTEGER, -- Sem foreign key por enquanto
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
...
-- Create table: agendamentos_fixos
CREATE TABLE agendamentos_fixos (
    id SERIAL PRIMARY KEY,
    espaco_id INTEGER NOT NULL REFERENCES espacos(id) ON DELETE CASCADE,
    usuario_id UUID NOT NULL REFERENCES usuarios(id) ON DELETE CASCADE,
...
CREATE INDEX idx_espacos_ativo ON espacos(ativo);
CREATE INDEX idx_espacos_nome ON espacos USING gin(nome gin_trgm_ops);

CREATE INDEX idx_agendamentos_data ON agendamentos(data);
CREATE INDEX idx_agendamentos_espaco_data ON agendamentos(espaco_id, data);
CREATE INDEX idx_agendamentos_usuario ON agendamentos(usuario_id);
CREATE INDEX idx_agendamentos_status ON agendamentos(status);
CREATE INDEX idx_agendamentos_composite ON agendamentos(espaco_id, data, status);

CREATE INDEX idx_agendamentos_fixos_espaco ON agendamentos_fixos(espaco_id);
CREATE INDEX idx_agendamentos_fixos_periodo ON agendamentos_fixos(data_inicio, data_fim);
CREATE INDEX idx_agendamentos_fixos_ativo ON agendamentos_fixos(ativo);
...
-- Adicionar foreign key constraint após criação das tabelas
ALTER TABLE agendamentos 
ADD CONSTRAINT fk_agendamentos_agendamento_fixo 
FOREIGN KEY (agendamento_fixo_id) REFERENCES agendamentos_fixos(id) ON DELETE SET NULL;
